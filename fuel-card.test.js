...
test('render() — tombol "Lihat Detail" & "Koreksi" tetap reuse class button yang sudah ada (0 class baru)', () => {
  const { doc, els } = makeFakeDoc(['fuelIntelWrap', 'fuelIntelBody']);
  const insight = {
    ok: true, vehicleId: 'v1', name: 'Vario', emoji: '🏍️',
    current: { ok: true, kmPerLiter: 42, rpPerKm: 238 }, reminders: [],
  ...
  assert.equal((html.match(/class="btn btn-ghost btn-sm"/g) || []).length, 2);
  assert.match(html, /class="btn-row"/);
});
