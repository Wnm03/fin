...
function baseFields(overrides = {}) {
  return {
    ...
    'keuanganTab-kelola': { style: {} }, 'keuanganTab-tagihan': { style: {} }, 'keuanganTab-budget': { style: {} }, 'keuanganTab-utangpiutang': { style: {} }, 'keuanganTab-asetproyek': { style: {} }, 'keuanganTab-laporan': { style: {} },
    ...overrides,
  };
}
...
test('setKeuanganTab — el TIDAK diberikan (null): fallback pilih tombol sesuai urutan kelola(0)/tagihan(1)/budget(2)/utangpiutang(3)/asetproyek(4)/laporan(5), dari queryGroups', () => {
  const order = ['kelola', 'tagihan', 'budget', 'utangpiutang', 'asetproyek', 'laporan'];
  for (const target of order) {
    const btns = order.map(() => fakeTabBtn());
    const { ctx } = makeCtx({}, { queryGroups: { '#page-keuangan .cn-tab': btns } });
    ctx.setKeuanganTab(target);
    order.forEach((t, i) => {
      assert.equal(btns[i].classList.active, t === target, `tombol tab "${t}" (idx ${i}) salah state saat target="${target}"`);
    });
  }
});
